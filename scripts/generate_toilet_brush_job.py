#!/usr/bin/env python3
"""Generate the toilet-brush shopping job YAML via OFFICIAL Playground services
(North America region only). Mirrors generate_water_bottle_job.py but adds
--filter region=North America so every sampled persona lives in North America.

Usage:
  uv run python scripts/generate_toilet_brush_job.py [sample_size] [n_concurrent_trials] [job_slug]
"""
...
def main() -> None:
    ...
    job_slug = sys.argv[3] if len(sys.argv) > 3 else "survey-toilet-brush-choice-n{}".format(sample_size)

    pr.retrieve_personas = _retrieve_full
    gen.retrieve_personas = _retrieve_full
    sys.argv = [
        "generate_toilet_brush_job",
        "--task", "application/tasks/survey_toilet-brush-choice",
        "--execution-mode", "auto",
