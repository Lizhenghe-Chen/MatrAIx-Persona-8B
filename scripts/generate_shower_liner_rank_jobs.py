#!/usr/bin/env python3
"""Generate rank-position job YAMLs via OFFICIAL Playground services (amazon, NA, seed 42)."""
...
ARMS = {"a-first": "A B C D E F", "c-first": "C A B D E F", "f-first": "F A B C D E"}
...
def main() -> None:
    ...
    for arm, order in ARMS.items():
        task = f"application/tasks/survey_shower-liner-6asin-rank-{arm}"
        slug = f"survey-shower-liner-rank-{arm}-n{sample_size}"
        sys.argv = [
            "generate_shower_liner_rank_jobs",
            "--task", task, "--execution-mode", "auto",
        ...
        gen.main()
        print(f"[generated] {slug} (order: {order})")
