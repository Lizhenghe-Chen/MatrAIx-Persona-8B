...
def _semantic_check(payload: dict[str, object]) -> str | None:
    ...
    for answer in answers:
        if isinstance(answer, dict) and str(answer.get("questionId", "")).strip() == "q_choice":
            choice = answer
            break
    if choice is None:
        return "missing q_choice answer"
    value = str(choice.get("value", "")).strip()
    if value not in ALLOWED_CHOICES:
        return "q_choice value '{}' not in allowed set".format(value)
    rationale = str(choice.get("rationale") or "").strip()
    if len(rationale) < 10:
        return "q_choice rationale is missing or too short"
    return None
