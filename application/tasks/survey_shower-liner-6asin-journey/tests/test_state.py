...
def _semantic_check(payload: dict[str, object]) -> str | None:
    ...
    for answer in answers:
        if isinstance(answer, dict) and str(answer.get("questionId", "")).strip() == "q_first_click":
            choice = answer
            break
    if choice is None:
        return "missing q_first_click answer"
    value = str(choice.get("value", "")).strip()
    if value not in ALLOWED_CHOICES:
        return "q_first_click value '{}' not in allowed set".format(value)
    rationale = str(choice.get("rationale") or "").strip()
    if len(rationale) < 10:
        return "q_first_click rationale is missing or too short"
    return None
...
def main() -> int:
    ...
    semantic_error = _semantic_check(payload)
    if semantic_error:
        return fail("semantic: {}".format(semantic_error))
